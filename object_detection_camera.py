...
def main():
    ...
    with picamera.PiCamera() as camera:
        ...
        camera.start_preview(fullscreen=False, window=(
            700, 200, FULL_SIZE_W, FULL_SIZE_H))
            ...
            for foo in camera.capture_continuous(stream,
                ...
                if padding_w > 0 or padding_h > 0:
                    flattened = pad_and_flatten(
                        input, (valid_resize_h, valid_resize_w), padding_h, padding_w)
                else:
                    flattened = input
                ...
                if args.draw:
                    ...
                    imbytes = img.tobytes()
                    if renderer == None:
                        renderer = camera.add_overlay(
                            imbytes, size=img.size, layer=4, format='rgba', fullscreen=False, window=(700, 200, 640, FULL_SIZE_H))
                    else:
                        #  print('updating')
                ...
                #  time.sleep(1)
                camera.annotate_text = "%.2fms, %d fps" % (
